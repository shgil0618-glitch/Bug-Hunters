...
export default function Mypage(){
    ...
    // 채팅방 생성 및 이동 함수
    const onStartChat = async (opponentId) => {
        try {
            // 1. 채팅방 생성 요청 (이미 있으면 기존 방 반환하도록 백엔드 구성 권장)
            const response = await api.post("/api/chat/room", {
                user1Id: user.id,   // 나
                user2Id: opponentId // 상대방
            });
    ...
    // 마이페이지나 채팅 리스트 상단에 추가할 로직
    // 채팅방 생성 요청 함수
    const onChat = (targetUserId) => {
        if (!user) return message.error("로그인이 필요합니다.");

        dispatch({
            type: "chat/createRoomRequest", // Saga에서 이 타입을 기다리고 있어야 함!
            payload: {
                user1Id: user.id,
                    ...
                    <List
                        ...
                        renderItem={(item) => (
                            <List.Item actions={[
                                <Button 
                                    key="chat" 
                                    onClick={() => onStartChat(item.followeeId)} // 여기 이름을 onChat으로 맞춤!
                                >
                                    메시지
                                </Button>,
                                <Button
