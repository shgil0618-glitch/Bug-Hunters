...
/////////////////////////////////////////////////회원가입////////////////////////////////////////////////////////
         ...
         request.getRequestDispatcher("view/UserJoin.jsp").forward(request, response);
          ...
          if (result == -1) {
              out.println("<script>alert('이미 사용 중인 이메일입니다.'); history.go(-1);</script>");
          } else if (result == -3) {
              out.println("<script>alert('닉네임은 한글/영문/숫자 2~10글자만 가능합니다.'); history.go(-1);</script>");
          } else if (result == -4) {
              out.println("<script>alert('비밀번호는 영문, 숫자, 특수문자를 포함한 8~20자리여야 합니다.'); history.go(-1);</script>");
          } else if (result == -5) {
              out.println("<script>alert('이메일 형식이 올바르지 않습니다.'); history.go(-1);</script>");
          } else if (result == -6) {
              out.println("<script>alert('휴대폰 번호 형식이 올바르지 않습니다.'); history.go(-1);</script>");
          } else if (result > 0) {
